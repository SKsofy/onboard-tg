...
пост в боте/канале
  ...
  → Экран 1 «Вау»: плитка до/после по всем сценариям (сценарий поста первым)
  → Экран 2 «Пейволл»: крупный личный таймер промокода (24 ч с первого
    захода, переживает перезагрузку) + оффер 49 р./3 дня + отзывы + FAQ
...
- А/Б-флаг: `INTERCEPT_DISCOUNT` (InterceptSheet.tsx, скидка 39 р. только
  ...
  `PROMO_HOURS` в data.ts — 24 ч с первого захода).
